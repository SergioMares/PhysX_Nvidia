...
Fireworks::Fireworks(Vector3 _EmitterPos, bool _bHasPayload, unsigned _PartType)
{
	...
	primigenial = new Particle(emitter, vPower, Size * 0.7, 1,1,damp);
}
...
void Fireworks::SpawnFireworks(int _Amount, Vector3 _Origin)
{
	...
	{
		newFW.push_back(new Fireworks(_Origin, false, partType));
	}
}
...
void Fireworks::Clear()
{
	...
	if (bSpawn && bPayload)
	{
		for (auto fw : newFW)
		{
			if (isfinite(fw->primigenial->getPos().p.x))
				delete fw->primigenial;
		}
	}
